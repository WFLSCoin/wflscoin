...
Syntax: `getcoins.py [-h|--help] [-c|--cmd=<wflscoin-cli path>] [-f|--faucet=<faucet URL>] [-a|--addr=<signet bech32 address>] [-p|--password=<faucet password>] [--] [<wflscoin-cli args>]`

* `--cmd` lets you customize the wflscoin-cli path. By default it will look for it in the PATH
* `--faucet` lets you specify which faucet to use; the faucet is assumed to be compatible with https://github.com/kallewoof/wflscoin-faucet
* `--addr` lets you specify a Signet address; by default, the address must be a bech32 address. This and `--cmd` above complement each other (i.e. you do not need `wflscoin-cli` if you use `--addr`)
...
To mine the first block in your custom chain, you can run:
    ...
    CLI="./wflscoin-cli -conf=mysignet.conf"
