...
This guide describes how to build wflscoind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Wflscoin Core from source.
...
### 4. Clone Wflscoin repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Wflscoin Core repository to a directory.
...
It is not necessary to build wallet functionality to run `wflscoind` or  `wflscoin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/wflscoin/wflscoin/blob/master/doc/descriptors.md), `sqlite` is required.
...
Wflscoin Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/wflscoin/wflscoin/issues/7714).
...
You can deploy a `.dmg` containing the Wflscoin Core application using `make deploy`.
...
## Building Wflscoin Core

### 1. Configuration

There are many ways to configure Wflscoin Core, here are a few common examples:
...
Run the following in your terminal to compile Wflscoin Core:
...
## Running Wflscoin Core

Wflscoin Core should now be available at `./src/wflscoind`.
If you compiled support for the GUI, it should be available at `./src/qt/wflscoin-qt`.

The first time you run `wflscoind` or `wflscoin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Wflscoin/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/Wflscoin"

touch "/Users/${USER}/Library/Application Support/Wflscoin/wflscoin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/Wflscoin/wflscoin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/Wflscoin/debug.log
```

## Other commands:

```shell
./src/wflscoind -daemon      # Starts the wflscoin daemon.
./src/wflscoin-cli --help    # Outputs a list of command-line options.
./src/wflscoin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/wflscoin-qt -server # Starts the wflscoin-qt server mode, allows wflscoin-cli control
```
