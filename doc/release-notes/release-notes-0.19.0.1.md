Wflscoin Core version 0.19.0.1 is now available from:

  <https://wflscoincore.org/bin/wflscoin-core-0.19.0.1/>
...
installer (on Windows) or just copy over `/Applications/Wflscoin-Qt` (on Mac)
or `wflscoind`/`wflscoin-qt` (on Linux).

Upgrading directly from a version of Wflscoin Core that has reached its EOL is
possible, but might take some time if the datadir needs to be migrated.  Old
wallet versions of Wflscoin Core are generally supported.

Compatibility
==============

Wflscoin Core is supported and extensively tested on operating systems using
the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not recommended
to use Wflscoin Core on unsupported systems.
...
to open the Wflscoin Core .dmg. This is due to new signing requirements
imposed by Apple, which the Wflscoin Core project does not yet adhere too.
...
- [Reduce memory](https://github.com/wflscoin/wflscoin/blob/master/doc/reduce-memory.md)
  suggests configuration tweaks for running Wflscoin Core on systems with
...
- `-blockfilterindex` enables the creation of BIP158 block filters for
  ...
  currently use about 4 GiB of space.  Note: this version of Wflscoin
...
- `whitebind` and `whitelist` now accept a list of permissions to
  ...
  releases.  See the `wflscoind -help` output for these two options for
...
- In 0.18.0, a `./configure` flag was introduced to allow disabling BIP70 support in the GUI (support was enabled by default). In 0.19.0, this flag is now __disabled__ by default. If you want to compile Wflscoin Core with BIP70 support in the GUI, you can pass `--enable-bip70` to `./configure`. (#15584)
...
- `generate` is now removed after being deprecated in Wflscoin Core 0.18.
...
- BIP 61 reject messages were deprecated in v0.18. They are now disabled
  ...
  future version of Wflscoin Core. (#14054)

- To eliminate well-known denial-of-service vectors in Wflscoin Core,
  ...
  This prevents Wflscoin Core from sending the BIP111 NODE_BLOOM service
  ...
  of Wflscoin Core and nodes that have manually enabled BIP37 support,
...
- By default, Wflscoin Core will now make two additional outbound connections that are exclusively used for block-relay.  No transactions or addr messages will be processed on these connections. These connections are designed to add little additional memory or bandwidth resource requirements but should make some partitioning attacks more difficult to carry out. (#15759)

Miscellaneous CLI Changes
-------------------------

- The `testnet` field in `wflscoin-cli -getinfo` has been renamed to
...
- `getblockchaininfo` no longer returns a `bip9_softforks` object.
  ...
  an additional `type` field describes how Wflscoin Core determines
...
- When fetching a transaction announced by multiple peers, previous versions of
  Wflscoin Core would sequentially attempt to download the transaction from each
...
- If a Tor hidden service is being used, Wflscoin Core will be bound to
  the standard port 9357 even if a different port is configured for
...
- Transactions with outputs paying v1 to v16 witness versions (future
  ...
  and services can accept any valid bech32 Wflscoin address without
...
- When creating a transaction with a fee above `-maxtxfee` (default 0.1
  WFLS), the RPC commands `walletcreatefundedpsbt` and
  `fundrawtransaction` will now fail instead of rounding down the fee.
  Be aware that the `feeRate` argument is specified in WFLS per 1,000
...
- A new wallet flag `avoid_reuse` has been added (default off). When
  ...
  partial spends" (added in Wflscoin Core v0.17.0), this can eliminate a
...
- #15649 Add ChaCha20Poly1305@Wflscoin AEAD (jonasschnelli)
...
- #15639 wflscoin-wallet tool: Drop libwflscoin_server.a dependency (ryanofsky)
...
- #16521 Use the default maxfeerate value as WFLS/kB (Remagpie)
...
- #16366 init: Use InitError for all errors in wflscoind/qt (MarcoFalke)
...
- #15239 scripts and tools: Move non-linux build source tarballs to "wflscoin-binaries/version" directory (hebasto)
...
- #16258 [MSVC]: Create the config.ini as part of wflscoind build (NicolasDorier)
...
- #16441 remove qt libjpeg check from wflscoin_qt.m4 (fanquake)
...
- #16750 msbuild: adds bench_wflscoin to auto generated project files (sipsorcery)
...
- #15773 Add WflscoinTestFramework::sync_* methods (MarcoFalke)
...
- #15352 Reduce noise level in test_wflscoin output (practicalswift)
...
- #15896 feature_filelock, interface_wflscoin_cli: Use PACKAGE_NAME in messages rather than hardcoding Wflscoin Core (luke-jr)
...
- #16277 Suppress output in test_wflscoin for expected errors (gertjaap)
...
- #16929 follow-up to rpc: default maxfeerate value as WFLS/kB (jonatack)
...
- #16489 log: harmonize wflscoind logging (jonatack)
...
- #17449 fix uninitialized variable nMinerConfirmationWindow (wflscoinVBR)

### Documentation
- #15514 Update Transifex links (fanquake)
- #15513 add "sections" info to example wflscoin.conf (fanquake)
...
- #15683 Comment for seemingly duplicate LIBWFLSCOIN_SERVER (Bushstar)
...
- #16080 build/doc: update wflscoin_config.h packages, release process (jonatack)
...
- #16448 add note on precedence of options in wflscoin.conf (fanquake)
...
- #16621 add default wflscoin.conf locations (GChuf)
...
- #16863 Add a missing closing parenthesis in the wflscoin-wallet's help (darosior)
- #16757 CChainState return values (MarcoFalke)
- #16847 add comments clarifying how local services are advertised (jamesob)
- #16812 Fix whitespace errs in .md files, wflscoin.conf, and Info.plist.in (ch4ot1c)
...
- #16912 Remove Doxygen intro from src/wflscoind.cpp (ch4ot1c)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/wflscoin/wflscoin/).
