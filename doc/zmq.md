...
The Wflscoin Core daemon can be configured to act as a trusted "border
router", implementing the wflscoin wire protocol and relay, making
...
The ZeroMQ feature in Wflscoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building wflscoind:
...
For instance:
    ...
    $ wflscoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/wflscoind.tx.raw \
...
These options can also be provided in wflscoin.conf.
...
From the perspective of wflscoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into wflscoind directly. Furthermore, no information is
...
using. Wflscoind appends an up-counting sequence number to each
