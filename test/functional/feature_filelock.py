...
"""Check that it's not possible to start a second wflscoind instance using the same datadir or wallet."""
import os
import random
import string
...
class FilelockTest(WflscoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Check that we can't start a second wflscoind instance using the same datadir")
        ...
        if self.is_wallet_compiled():
            def check_wallet_filelock(descriptors):
                ...
                self.log.info("Check that we can't start a second wflscoind instance using the same wallet")
                if descriptors:
                    expected_msg = "Error: SQLiteDatabase: Unable to obtain an exclusive lock on the database, is it being used by another wflscoind?"
                else:
