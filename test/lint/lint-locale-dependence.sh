...
# Be aware that wflscoind and wflscoin-qt differ in terms of localization: Qt
# opts in to POSIX localization by running setlocale(LC_ALL, "") on startup,
# whereas no such call is made in wflscoind.
#
...
# C locale.
#
# In contrast, wflscoind does not opt in to localization -- no call to
# setlocale(LC_ALL, "") is made and the environment variables LC_* are
# thus ignored.
#
# This results in situations where wflscoind is guaranteed to be running
# with the classic locale ("C") whereas the locale of wflscoin-qt will vary
# depending on the user's environment variables.
#
# An example: Assuming the environment variable LC_ALL=de_DE then the
# call std::to_string(1.23) will return "1.230000" in wflscoind but
# "1,230000" in wflscoin-qt.
#
...
KNOWN_VIOLATIONS=(
    "src/wflscoin-tx.cpp.*stoul"
    "src/wflscoin-tx.cpp.*trim_right"
