...
const char * const WFLSCOIN_CONF_FILENAME = "wflscoin.conf";
const char * const WFLSCOIN_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/wflscoin/wflscoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //wflscoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", WFLSCOIN_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Wflscoin
    // macOS: ~/Library/Application Support/Wflscoin
    // Unix-like: ~/.wflscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Wflscoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Wflscoin";
#else
    // Unix-like
    return pathRet / ".wflscoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", WFLSCOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Wflscoin Core copyright is not removed by accident
    if (copyright_devs.find("Wflscoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Wflscoin Core developers";
    }
