This directory contains the source code for the Wflscoin Core graphical user interface (GUI). It uses the [Qt](https://www1.qt.io/developers/) cross-platform framework.
...
- Represents the main window of the Wflscoin UI.
...
- ClientModel is used by the main application `wflscoingui` and several models like `peertablemodel`.
...
- (Deprecated) Used to process BIP21 payment URI requests. Also handles URI-based application switching (e.g. when following a wflscoin:... link from a browser).
...
* UI elements like WflscoinAmountField, which inherit from QWidget.
* `wflscoinstrings.cpp`: automatically generated
* `wflscoinunits.(h/cpp)`: WFLS / mWFLS / etc. handling
* `callback.h`
...
**Note:** Do not change `local/wflscoin_en.ts`. It is updated [automatically](/doc/translation_process.md#writing-code-with-translations).
...
4. Enter `wflscoin-qt` as the Project Name and enter the absolute path to `src/qt` as Location
...
8. While in the `Projects` tab, ensure that you have the `wflscoin-qt` executable specified under `Run`
 - If the executable is not specified: click `"Choose..."`, navigate to `src/qt`, and select `wflscoin-qt`
9. You're all set! Start developing, building, and debugging the Wflscoin Core GUI
