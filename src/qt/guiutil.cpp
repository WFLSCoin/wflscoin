...
#include <qt/wflscoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Wflscoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new WflscoinAddressEntryValidator(parent));
    widget->setCheckValidator(new WflscoinAddressCheckValidator(parent));
}
...
bool parseWflscoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no wflscoin: URI
    if(!uri.isValid() || uri.scheme() != QString("wflscoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!WflscoinUnits::parse(WflscoinUnits::WFLS, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseWflscoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseWflscoinURI(uriInstance, out);
}

QString formatWflscoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("wflscoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(WflscoinUnits::format(WflscoinUnits::WFLS, info.amount, false, WflscoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openWflscoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", WFLSCOIN_CONF_FILENAME));
    ...
    /* Open wflscoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Wflscoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Wflscoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Wflscoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Wflscoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "wflscoin.desktop";
    return GetAutostartDir() / strprintf("wflscoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a wflscoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Wflscoin\n";
        else
            optionFile << strprintf("Name=Wflscoin (%s)\n", chain);
