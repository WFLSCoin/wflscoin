...
uint256 SendCoins(CWallet& wallet, SendCoinsDialog& sendCoinsDialog, const CTxDestination& address, CAmount amount, bool rbf)
{
    ...
    entry->findChild<WflscoinAmountField*>("payAmount")->setValue(amount);
...
//
//     QT_QPA_PLATFORM=xcb     src/qt/test/test_wflscoin-qt  # Linux
//     QT_QPA_PLATFORM=windows src/qt/test/test_wflscoin-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   src/qt/test/test_wflscoin-qt  # macOS
void TestGUI(interfaces::Node& node)
{
...

    {
        ...
        QString balanceComparison = WflscoinUnits::formatWithUnit(unit, balance, false, WflscoinUnits::SeparatorStyle::ALWAYS);
    ...
    // Amount input
    WflscoinAmountField* amountInput = receiveCoinsDialog.findChild<WflscoinAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'QT_QPA_PLATFORM=cocoa test_wflscoin-qt' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    TestGUI(m_node);
