dnl Copyright (c) 2013-2016 The Wflscoin Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set wflscoin_enable_qt to false. Else, exit.
AC_DEFUN([WFLSCOIN_QT_FAIL],[
  if test "x$wflscoin_qt_want_version" = xauto && test "x$wflscoin_qt_force" != xyes; then
    if test "x$wflscoin_enable_qt" != xno; then
      AC_MSG_WARN([$1; wflscoin-qt frontend will not be built])
    fi
    wflscoin_enable_qt=no
    wflscoin_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])

AC_DEFUN([WFLSCOIN_QT_CHECK],[
  if test "x$wflscoin_enable_qt" != xno && test "x$wflscoin_qt_want_version" != xno; then
    true
    $1
  else
    true
...
dnl WFLSCOIN_QT_PATH_PROGS([FOO], [foo foo2], [/path/to/search/first], [continue if missing])
...
AC_DEFUN([WFLSCOIN_QT_PATH_PROGS],[
  WFLSCOIN_QT_CHECK([
    if test "x$3" != x; then
      AC_PATH_PROGS($1,$2,,$3)
    else
    ...
    if test "x$$1" = x && test "x$4" != xyes; then
      WFLSCOIN_QT_FAIL([$1 not found])
    fi
  ])
])

dnl Initialize qt input.
dnl This must be called before any other WFLSCOIN_QT* macros to ensure that
dnl input variables are set correctly.
dnl CAUTION: Do not use this inside of a conditional.
AC_DEFUN([WFLSCOIN_QT_INIT],[
  dnl enable qt support
  AC_ARG_WITH([gui],
    [AS_HELP_STRING([--with-gui@<:@=no|qt5|auto@:>@],
    [build wflscoin-qt GUI (default=auto)])],
    [
     wflscoin_qt_want_version=$withval
     if test "x$wflscoin_qt_want_version" = xyes; then
       wflscoin_qt_force=yes
       wflscoin_qt_want_version=auto
     fi
    ],
    [wflscoin_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); wflscoin_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl
dnl   WFLSCOIN_QT_CONFIGURE([MINIMUM-VERSION])
dnl
dnl Outputs: See _WFLSCOIN_QT_FIND_LIBS
dnl Outputs: Sets variables for all qt-related tools.
dnl Outputs: wflscoin_enable_qt, wflscoin_enable_qt_dbus, wflscoin_enable_qt_test
AC_DEFUN([WFLSCOIN_QT_CONFIGURE],[
  qt_version=">= $1"
  qt_lib_prefix="Qt5"
  WFLSCOIN_QT_CHECK([_WFLSCOIN_QT_FIND_LIBS])
  ...
  dnl the final binary as well. _WFLSCOIN_QT_CHECK_STATIC_PLUGIN does
  ...
  _WFLSCOIN_QT_IS_STATIC
  if test "x$wflscoin_cv_static_qt" = xyes; then
    _WFLSCOIN_QT_CHECK_STATIC_LIBS
    ...
    if test "x$TARGET_OS" != xandroid; then
      _WFLSCOIN_QT_CHECK_STATIC_PLUGIN([QMinimalIntegrationPlugin], [-lqminimal])
    ...
    if test "x$TARGET_OS" = xwindows; then
      ...
      _WFLSCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsIntegrationPlugin], [-lqwindows])
      _WFLSCOIN_QT_CHECK_STATIC_PLUGIN([QWindowsVistaStylePlugin], [-lqwindowsvistastyle])
      ...
      _WFLSCOIN_QT_CHECK_STATIC_PLUGIN([QXcbIntegrationPlugin], [-lqxcb])
      ...
      _WFLSCOIN_QT_CHECK_STATIC_PLUGIN([QCocoaIntegrationPlugin], [-lqcocoa])
      _WFLSCOIN_QT_CHECK_STATIC_PLUGIN([QMacStylePlugin], [-lqmacstyle])
  ...
  WFLSCOIN_QT_PATH_PROGS([MOC], [moc-qt5 moc5 moc], $qt_bin_path)
  WFLSCOIN_QT_PATH_PROGS([UIC], [uic-qt5 uic5 uic], $qt_bin_path)
  WFLSCOIN_QT_PATH_PROGS([RCC], [rcc-qt5 rcc5 rcc], $qt_bin_path)
  WFLSCOIN_QT_PATH_PROGS([LRELEASE], [lrelease-qt5 lrelease5 lrelease], $qt_bin_path)
  WFLSCOIN_QT_PATH_PROGS([LUPDATE], [lupdate-qt5 lupdate5 lupdate],$qt_bin_path, yes)
  WFLSCOIN_QT_PATH_PROGS([LCONVERT], [lconvert-qt5 lconvert5 lconvert], $qt_bin_path, yes)
  ...
  WFLSCOIN_QT_CHECK([
    wflscoin_enable_qt=yes
    wflscoin_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      wflscoin_enable_qt_test=no
    fi
    wflscoin_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      wflscoin_enable_qt_dbus=yes
    fi
  ...
  if test x$wflscoin_enable_qt = xyes; then
    AC_MSG_RESULT([$wflscoin_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$wflscoin_enable_qt])
  fi
...
dnl
dnl _WFLSCOIN_QT_IS_STATIC
dnl ---------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
dnl Output: wflscoin_cv_static_qt=yes|no
AC_DEFUN([_WFLSCOIN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, wflscoin_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [wflscoin_cv_static_qt=yes],
      [wflscoin_cv_static_qt=no])
    ])
])

dnl Internal. Check if the link-requirements for a static plugin are met.
dnl
dnl _WFLSCOIN_QT_CHECK_STATIC_PLUGIN(PLUGIN, LIBRARIES)
dnl --------------------------------------------------
dnl
dnl Requires: INCLUDES and LIBS must be populated as necessary.
...
AC_DEFUN([_WFLSCOIN_QT_CHECK_STATIC_PLUGIN], [
  ...
  AC_LINK_IFELSE([AC_LANG_PROGRAM([[
    ...
    [AC_MSG_RESULT([no]); WFLSCOIN_QT_FAIL([$1 not found.])])
  LIBS="$CHECK_STATIC_PLUGINS_TEMP_LIBS"
])

dnl Internal. Check Qt static libs with PKG_CHECK_MODULES.
dnl
dnl _WFLSCOIN_QT_CHECK_STATIC_LIBS
dnl -----------------------------
dnl
dnl Outputs: QT_LIBS is prepended.
AC_DEFUN([_WFLSCOIN_QT_CHECK_STATIC_LIBS], [
...
dnl
dnl _WFLSCOIN_QT_FIND_LIBS
dnl ---------------------
dnl
dnl Outputs: All necessary QT_* variables are set.
dnl Outputs: have_qt_test and have_qt_dbus are set (if applicable) to yes|no.
AC_DEFUN([_WFLSCOIN_QT_FIND_LIBS],[
  WFLSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_CORE], [${qt_lib_prefix}Core${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_CORE_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_CORE_LIBS $QT_LIBS"],
                      [WFLSCOIN_QT_FAIL([${qt_lib_prefix}Core${qt_lib_suffix} $qt_version not found])])
  ])
  WFLSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_GUI], [${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_GUI_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_GUI_LIBS $QT_LIBS"],
                      [WFLSCOIN_QT_FAIL([${qt_lib_prefix}Gui${qt_lib_suffix} $qt_version not found])])
  ])
  WFLSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_WIDGETS], [${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_WIDGETS_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_WIDGETS_LIBS $QT_LIBS"],
                      [WFLSCOIN_QT_FAIL([${qt_lib_prefix}Widgets${qt_lib_suffix} $qt_version not found])])
  ])
  WFLSCOIN_QT_CHECK([
    PKG_CHECK_MODULES([QT_NETWORK], [${qt_lib_prefix}Network${qt_lib_suffix} $qt_version], [QT_INCLUDES="$QT_NETWORK_CFLAGS $QT_INCLUDES" QT_LIBS="$QT_NETWORK_LIBS $QT_LIBS"],
                      [WFLSCOIN_QT_FAIL([${qt_lib_prefix}Network${qt_lib_suffix} $qt_version not found])])
  ])
